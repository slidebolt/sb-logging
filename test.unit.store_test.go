...
func newSQLiteStore(t *testing.T) logging.Store {
	t.Helper()
	svc, err := server.New(logcfg.Config{Target: "sqlite", SQLitePath: t.TempDir() + "/logs.db"})
	if err != nil {
...
func TestMemoryStoreAppendGetAndList(t *testing.T) {
	for _, tc := range []struct {
		name  string
		store func(*testing.T) logging.Store
	}{
		{name: "memory", store: newMemoryStore},
		{name: "sqlite", store: newSQLiteStore},
	} {
...
func TestMemoryStorePrunesOldestEventsAtDefaultCap(t *testing.T) {
	...
	if len(events) != 1000 {
		t.Fatalf("List len: got %d want %d", len(events), 1000)
	}
	if events[0].TS != base.Add(1*time.Second) {
		t.Fatalf("oldest ts: got %s want %s", events[0].TS, base.Add(1*time.Second))
	}
	if _, err := store.Get(ctx, "evt-0000"); !errors.Is(err, logging.ErrNotFound) {
		t.Fatalf("Get(pruned): got %v want ErrNotFound", err)
	}
}
